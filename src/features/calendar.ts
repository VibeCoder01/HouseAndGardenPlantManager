...
function collectTasksForMonth(
  ...
  const fertiliseTask = computeFertiliseTasks(plant, monthStart, override, options.winterMonths);
  if (fertiliseTask) {
...
function computeFertiliseTasks(
  ...
  if (fertilise.cadence === "monthly") {
    const target = addDaysToDate(monthStart, 14);
    dueDates.push(formatYMD(target));
  } else {
    // every watering quarter strength -> piggy-back on watering hints
    const waterTask = computeWaterTasks(plant, monthStart, addDaysToDate(monthStart, 27), override);
    if (waterTask) {
  ...
  if (!dueDates.length) {
    const fallback = addDaysToDate(monthStart, 21);
...
function computeRepotTask(
  ...
  if (lastRepot) {
    const diffYears = monthStart.getFullYear() - lastRepot.getFullYear();
    if (diffYears < 1) {
      return null;
    }
  }
  ...
  return {
    ...
    intensity: "normal",
    note: repot.guidance === "spring_preferred" ? "Spring repot window" : "Review pot size",
  };
}
